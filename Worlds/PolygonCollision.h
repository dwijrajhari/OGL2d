...
class PolygonCollision : public World
{
	Polygon* p1, * p2, * pc;
...
public:
	...
	void fSetupState(int SW_, int SH_) override
	{
		...
		p2 = new Polygon(Point(-200.0), 75.0, 3);
	...
	void fKeyHandleInput(int key, int scancode, int action, int mods) override
	{
		...
		if (key == GLFW_KEY_SPACE && action == GLFW_RELEASE) pc = pc == p1 ? p2 : p1;
	}
	...
	bool fCheckPointInPoly(Point pt, Polygon py)
	{
